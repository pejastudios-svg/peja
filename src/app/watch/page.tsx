...
export default function ProfilePage() {
  ...
  // --- 2. DISABLE BROWSER SCROLL RESTORATION (BEFORE ANYTHING ELSE) ---
  useLayoutEffect(() => {
    if (typeof window !== "undefined") {
  ...
  // --- 3. SCROLL RESTORE (AFTER CONTENT LOADS) ---
  useLayoutEffect(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        // Start tracking after a short delay to avoid catching any residual scroll events
        setTimeout(() => {
  ...
  // --- 4. TRACK SCROLL WITH STABILITY CHECK ---
  useEffect(() => {
    const onScroll = () => {
      ...
      // Use stability timeout - only save "stable" positions
      // This prevents saving wrong positions during navigation transitions
      clearTimeout(stabilityTimeoutRef.current);
      stabilityTimeoutRef.current = setTimeout(() => {
        lastStableScrollRef.current = currentY;
        // Save to cache immediately when scroll stabilizes
    ...
    return () => {
      ...
      clearTimeout(stabilityTimeoutRef.current);
      // On unmount, save the LAST STABLE position, not the current one
      // This avoids saving jump positions that happen during navigation
